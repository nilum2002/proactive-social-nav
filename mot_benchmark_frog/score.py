...
# start < end -- so a perfectly normal single-frame association crashes the whole run. At
# FROG's 50k-scan scale this is not an edge case; it happens routinely. Nudge a degenerate
# datetime range's end forward by 1us (below FROG's ~25ms scan spacing, so it can never
# reach the next real timestamp) instead of raising.
...
def build_truths(gt, rows):
    ...
    Returns a list sorted by id, not a set. `ClearMotAssociator` only ever
    iterates this (no set-only ops), and a plain Python `set` of these objects
    hashes by identity -- its iteration order depends on object memory
    addresses, which differ between process runs. That silently changes which
    of several equal-cost pairings the Hungarian assignment in
    `_match_unassigned_tracks` picks on an exact-distance tie, so re-running
    the identical scoring code on the identical dump could report a different
    MOTA. Sorting by id makes the pooled FN/FP/IDSW/MOTA/MOTP numbers this
    script prints fully reproducible across runs. (The `score_segment`
    cross-check against Stone Soup's own `ClearMotMetrics` can still show a
    tiny residual delta on a tie: `MultiManager.add_data` rewraps whatever it
    is given back into a `set(value)` internally, so that path's ordering
    isn't controllable from here -- harmless to these headline numbers.)
    """
...
def build_tracks(tr, seg):
    ...
    Returns a list sorted by id, not a set -- see the note in `build_truths`.
    """
    m = tr["segment"] == seg
    by_id = defaultdict(list)
...
def main(argv=None):
    ...
    out_path = os.path.join(p["results_dir"], f"{p['bag']}_{stamp}.json")
