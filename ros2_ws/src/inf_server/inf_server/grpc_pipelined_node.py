...
from inf_server.grpc_server_node import (
    InfServerNode,
    PerceptionServicer,
...
class StageStats:
    ...
    def record_detect(self, det_s, gap_s, depth):
        with self._lock:
    ...
    def drain(self):
        with self._lock:
            snapshot = (
                list(self.det_s), list(self.track_s), list(self.lat_s),
                list(self.scan_gap_s), list(self.q_depth), self.q_full_events,
            )
            self.reset()
        return snapshot
    ...
    def StreamSensorData(self, request_iterator, context):
        ...
        try:
            for frame in request_iterator:
                ...
                node.stats.record_detect(det_s, gap, work_q.qsize())
...
class PipelinedInfServerNode(InfServerNode):
    ...
    def _log_status(self):
        ...
        det, track, lat, gap, depth, q_full = self.stats.drain()
