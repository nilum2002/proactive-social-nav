...
from benchmark.udp_server_node import UdpServerNode
...
class PipelinedUdpServerNode(UdpServerNode):
    ...
    def _log_status(self):
        # The base class prints the session/link line plus the shared T_* block
...
def main(args=None):
    ...
    try:
        ...
        print(f"Error starting benchmark udp pipelined node: {e}")
    finally:
        if node is not None:
            node.shutdown()
            node.destroy_node()
