...
QoS is the entire substance of the DDS transport -- it is what separates "DDS
over WiFi" from "DDS over WiFi done in the way that actually works". A silently
...
possible failure for a benchmark.

Worse on this side than on the client's: a RELIABLE subscriber matched against
a BEST_EFFORT publisher receives nothing at all, with no error anywhere, which
is indistinguishable from a robot that was never switched on. Hence tests
rather than a successful run.
"""
import pytest
from rclpy.qos import DurabilityPolicy, HistoryPolicy, ReliabilityPolicy

from inf_server_wifi.wifi_server_node import build_qos
...
def test_best_effort_is_default_shape():
    ...
    assert qos.depth == 1
    # VOLATILE matters as much as BEST_EFFORT: TRANSIENT_LOCAL would hand this
    # node a stale scan as its first sample on every reconnect, and the tracker
    # would start from a pose the robot left minutes ago.
...
def test_depth_is_floored_at_one(depth):
    ...
    # let the middleware reject the subscription at construction time.
