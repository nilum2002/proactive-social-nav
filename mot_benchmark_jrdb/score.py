"""Score replayed JRDB tracks against real-identity ground truth with CLEAR
MOT (MOTA / MOTP). Identical accounting to mot_benchmark_frog/score.py --
same Stone Soup 1.9.1 zero-length TimeRange patch, same deterministic-
ordering fix for build_truths/build_tracks (both bugs are unrelated to
which dataset is being scored). See that file for the full rationale.
"""
import argparse
...
from jrdb_common import load_config, DEFAULT_CONFIG
...
def build_truths(gt, seg):
    """One GroundTruthPath per JRDB identity within this segment (sequence).
    Returns a list sorted by id, not a set -- see mot_benchmark_frog/
    score.py's build_truths docstring for why order must be deterministic."""
    m = gt["segment"] == seg
    rows_in_seg = set(np.nonzero(m)[0].tolist())
    by_id = defaultdict(list)
    for scan_row, gid, (x, y) in zip(gt["gt_scan"], gt["gt_id"], gt["gt_xy"]):
        if int(scan_row) not in rows_in_seg:
            continue
...
def main(argv=None):
    ...
    dumps = sorted(glob.glob(os.path.join(p["out_dir"], "tracks_jrdb_*.npz")))
    if not dumps:
    ...
    for dump in dumps:
        ...
        for ad in cfg["scoring"]["association_distances"]:
            ...
            for seg in sorted(covered):
                truths = build_truths(gt, int(seg))
    ...
    out_path = os.path.join(p["results_dir"], f"jrdb_test_{stamp}.json")
