...
class _SpatialAttentionMemory(nn.Module):
    ...
    def forward(self, x_new):
        ...
        if self._memory.shape[1] != n_cutout:
            mem_resized = self._memory.permute(0, 2, 3, 1).view(n_batch, n_channel * n_pts, -1)
            mem_resized = F.interpolate(mem_resized, size=n_cutout, mode='linear', align_corners=False)
            self._memory = mem_resized.view(n_batch, n_channel, n_pts, n_cutout).permute(0, 3, 1, 2)
